                ...
                <Container>
                    ...
                    {activeStep === 0 && (
                        <div>
                            ...
                            {renderTextField("Date and Time of Lottery Draw", lotteryDetails.endDate, (e) => setLotteryDetails({ ...lotteryDetails, endDate: e.target.value }), "datetime-local")}
                    ...
                    {activeStep === 2 && (
                        <div>
                            <Typography variant="h4" sx={{ mt: 2, textAlign: "center", fontWeight: "bold" }}>
                                Select the conditions for the lottery
                            </Typography>
                        ...
                        <div>
                            ...
                            {lotteryDetails.prizes.map((prize, index) => (
                                <Box key={index} sx={{ mt: 2, display: "flex", alignItems: "center", gap: 2 }}>
                                    <Typography sx={{ minWidth: "70px", whiteSpace: "nowrap" }}>Place {index + 1}</Typography>
                                    {renderTextField("Prize Description", prize.description, (e) => handlePrizeChange(index, "description", e.target.value))}
                                    <Select value={prize.icon} onChange={(e) => handlePrizeChange(index, "icon", e.target.value)} displayEmpty sx={{ minWidth: 120 }}>
                                        <MenuItem value="" disabled>Select Icon</MenuItem>
                                        {iconOptions.map((option) => (<MenuItem key={option.value} value={option.value}>{option.icon} {option.label}</MenuItem>))}
                                    </Select>
                                </Box>
                            ))}
                            ...
                            <Box className="flexColCenter NavBut">
                                <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 2, mt: 2 }}>
                                    <button className="button button-green" onClick={prevStep}>Back</button>
                                    {lotteryDetails.prizes.some((prize) => prize.description && prize.icon) && <button className="button button-blue" onClick={handleFinish}>Finish</button>}
                                </Box>
                            </Box>
                        </div>
                    )}
